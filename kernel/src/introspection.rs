...
impl KernelInfo {
    ...
    pub fn number_app_grant_uses(&self, app: AppId,
        _capability: &dyn ProcessManagementCapability,) -> (usize, usize) {
        ...
        let mut used = 0;
        self.kernel
            .process_map_or((), app, |process| {
                for i in 0..number_of_grants {
                    unsafe {
                        let ctr_ptr = process.grant_ptr(i);
                        // If the pointer at that location is  not NULL then the
                        // grant memory has been allocated and the grant is being
                        // used.
                        if !(*ctr_ptr).is_null() {
                            used += 1;
                        }
